...
# get a random sample
random_content_sample = SMCCars[1]#[random.randint(0, len(SMCCars)-1)]
...
index = 3 #random.randint(0, new_sample_seg.size()[0]-1)

# add the channel to sample
# new_sample_seg[index] = torch.max(new_sample_seg[index], random_content_seg[index]) 

# new_image = torch.where(new_sample_seg[index] > 0, random_content_sample['image'], sample['image'])
